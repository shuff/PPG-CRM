   ...
   {// != false) //{
	...
	$timestamp = date("m/d/Y h:i:s a", time());
    	...
    	while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
			...
			$company_status_code = $data[2];
			If ($company_status_code == 'A'){$company_status_code = 'Activated';}
			...
			If ($company_status_code == 'W'){$company_status_code = 'Widthdrawn';}
...
*/
			...
			if ($amount_processed >= 1 && get_field_value('orders', 'payment_status', 'merchant_id',$merchant_id) == 'unpaid') {
